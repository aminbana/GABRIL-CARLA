...
class HumanInterface(object):
    ...
    def __init__(self, display_width, display_height, width, height, side_scale, left_mirror=False, right_mirror=False):
        ...
        self._display = pygame.display.set_mode((self.display_width, self.display_height), pygame.HWSURFACE | pygame.DOUBLEBUF)
    ...
    def run_interface(self, input_data):
        ...
        image_center = cv2.resize(image_center, (self.display_width, self.display_height))
        ...
        if self._left_mirror:
            ...
            image_left = cv2.resize(image_left, (int(self.display_width * self._scale), int(self.display_height * self._scale)))
            left_surface = pygame.surfarray.make_surface(image_left.swapaxes(0, 1))
            self._surface.blit(left_surface, (0, (1 - self._scale) * self.display_height))

        # Add the right mirror
        if self._right_mirror:
            image_right = input_data['Right'][1][:, :, -2::-1]
            image_right = cv2.resize(image_right, (int(self.display_width * self._scale), int(self.display_height * self._scale)))            
            right_surface = pygame.surfarray.make_surface(image_right.swapaxes(0, 1))
            self._surface.blit(right_surface, ((1 - self._scale) * self.display_width, (1 - self._scale) * self.display_height))
